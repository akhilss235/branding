...
const SCROLL_SPEED_MULTIPLIER = 1.48;
...
const MODEL_CAMERA_ROTATION_FACTOR = { x: 0.16, y: 1.7 };
  ...
  useEffect(() => {
    ...
    const updateModelRotationFromCamera = () => {
      ...
      activeModelAnchor.rotation.set(
        ...
        MODEL_ROTATION.y + yaw * MODEL_CAMERA_ROTATION_FACTOR.y,
        MODEL_ROTATION.z
      );
    };
    ...
    const updateCar = () => {
      ...
      const targetAngle =
        progress <= STANDING_SCROLL_THRESHOLD
          ? THREE.MathUtils.lerp(WATER_IDLE_ANGLE, angle, introEase)
          : angle;
