...
const frameSource = (index) =>
  `/macbook11/ezgif-frame-${String(index + 1).padStart(3, "0")}.png`;
const FEATURES = [
  {
    eyebrow: "Anoop Krishna / Digital Marketing Strategist",
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      for (let offset = 1; offset <= 10; offset += 1) {
        ...
        if (offset <= 3) requestFrame(Math.max(targetFrame - offset, 0));
      }

      setActiveFeature(Math.min(Math.floor(progress * FEATURES.length), FEATURES.length - 1));
    };
